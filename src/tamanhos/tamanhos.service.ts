...
export class TamanhosService {
  ...
  async create(
    createTamanhoDto: CreateTamanhoDto,
    dateUser: { id: number; name: string; empresa: string },
  ): Promise<Tamanho> {
    ...
    throw new BadRequestException(
      'Voce nao tem autorizacao para inserir um tamanho novo!!!',
    );
  }
  ...
  async remove(
    ...
    if (!dateUser.empresa && dateUser.empresa !== process.env.MASTER_NAME) {
      throw new BadRequestException(
        'Voce nao tem autorizacao para remove um tamanho!!!',
      );
    }
